...
export class AuthService {
  ...
  private AuthRepository: AuthRepository = container.resolve(AuthRepository)
  ...
  public getUserById = async (id: string): Promise<Users[]> => {
    try {
  ...
  public addUser = async (user: UserRegister): Promise<Users[]> => {
    try {
      const isEmailUnique = await this.AuthRepository.isEmailUnique(user.email)
      const isNameUnique = await this.AuthRepository.isNameUnique(user.name)
      ...
      if (!isEmailUnique) {
        ...
        return await this.AuthRepository.addUser(user)
      }
  ...
  public getPasswordByEmail = async (email: string): Promise<Users> => {
    try {
      return await this.AuthRepository.getPasswordByEmail(email)
    } catch (error) {
      throw new Error((error as Error).message)
    }
  }
