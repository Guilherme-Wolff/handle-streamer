...
import { IUsersRepository } from "../../domain/repository/IUsersRepository.js"
...
export class UsersRepository
  ...
  async getPasswordByEmail(email:string): Promise<Users | null> {
    //const user = await dataSource.manager.findOne<any>(Users, {email})
    const user = await dataSource.manager.findOne(Users,{where: { email }});
    return user;
  }
